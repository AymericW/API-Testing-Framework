...
When('I create more than twenty psd2 consents in {string}', (brand, callback) => {
    let headers = brand == "Fintro"? fintroLoadMoreHeader: fortisLoadMoreHeader;
    let clientIdList = ["k4QztmnS20", "NvWt0TLS20"]; 
        // ["k4QztmnS20", "NvWt0TLS20", "pWnsrsTS20", "ssI8usTS20",
    ...
    let accountNumberList = ["BE15001831411530"];

    // accountNumberList.forEach((accountNumber) => {
        clientIdList.forEach((clientId) => {
            getListOfAccounts(headers)
            .then(() => {
                console.log("yolo")
                createPsd2Consent(headers, clientId, accountNumberList[0])
            })
            .then(callback());
        })
    // })

    // for (i = 0; i < accountNumberList.length; i++) {
        // for (j = 0; j < clientIdList.length; j++) {
        //     getListOfAccounts(headers)
        //     .then(() => {
        //         console.log(clientIdList[j])
        //         createPsd2Consent(headers, clientIdList[j], accountNumberList[0])
        //     })
        //     .then(callback())
        //     .then(() => {
        //         console.log("Waiting...");
